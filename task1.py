...
def convolve2d(img, kernel):
    ...
    #img_sh_flip = utils.flip2d(img)
    kernel = utils.flip2d(kernel)

    
    #2. Pad the image with 0s
    #img_zeropadded = utils.zero_pad(img_sh_flip,1,1)
    img = utils.zero_pad(img,1,1)
     
    #Height of the image > Row - len(img)
    #Width of the image > Column - len of 0th row of img
    ...
    #Allocating final result image as a list with all 0s
    #CITATION: https://www.geeksforgeeks.org/initialize-matrix-in-python/
    
    img_conv = [[0 for r in range(col_image)] for s in range(row_image)]
    
    for i in range(row_image-2):
        for j in range(col_image-2):
            #multiply cropped zero padded image with kernel
                    
            cropped_img = utils.crop(img,i,i+3,j,j+3)
            product = utils.elementwise_mul(cropped_img,kernel)
                    
            sum1 = 0
            for m in range(len(product)):
                for n in range(len(product[m])):
                    sum1 += product[m][n]
...
def normalize(img):
    ...
    normalized_img = [[0 for r in range(col_image)] for s in range(row_image)]
    
    maximum = max([max(row) for row in img])
    minimum = min([min(row) for row in img])
    range1 = (maximum-minimum)
    
    for i in range(row_image):
        for j in range(col_image):
            normalized_img[i][j] = (255)*((img[i][j]-minimum)/range1)
...
def edge_magnitude(edge_x, edge_y):
    ...
    """
    #CITATION: https://github.com/Altron01/Sobel-operator-Edge-Detection/blob/master/sobelOperator.py
    
    #for (i in range 0,
    # TODO: implement this function.
    #raise NotImplementedError
    
    edge_magnitude = copy.deepcopy(edge_x)
    row_image = len(edge_magnitude)
    col_image = len(edge_magnitude[0])
    
    for i in range(row_image):
        for j in range(col_image):
            
            #Taking squares of x and y edges
            x = edge_x[i][j]**2
            y = edge_y[i][j]**2
            
            #taking squareroot
            edge_magnitude[i][j] = np.sqrt(x+y)
